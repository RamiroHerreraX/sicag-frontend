...
const EXPORT_OPTIONS = [
  { id: 'compliance', label: 'Cumplimiento por Región' },
  { id: 'certs',      label: 'Certificaciones' },
  { id: 'decl',       label: 'Declaraciones' },
  { id: 'committee',  label: 'Actividad Comité' },
  { id: 'assoc',      label: 'Asociaciones y Cumplimiento' },
];
...
const exportToPdf = async (selectedIds, filters, dateRanges, onProgress, expandedState, setExpandedState) => {
  ...
  pdf.setFillColor(19, 59, 107);
  pdf.rect(0, pH * 0.45, pW, pH * 0.55, 'F');
  pdf.setFillColor(0, 168, 168);
  pdf.roundedRect(margin, 48, 78, 8, 2, 2, 'F');
  pdf.setTextColor(255, 255, 255);
  pdf.setFontSize(8);
  pdf.setFont('helvetica', 'bold');
  pdf.text('SICAG · Sistema de Cumplimiento Gremial', margin + 4, 53.5);
  pdf.setFontSize(28);
  pdf.setFont('helvetica', 'bold');
  pdf.text('Reporte de Cumplimiento Gremial', margin, 76);
  pdf.setFontSize(11);
  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(200, 215, 235);
  pdf.text('Informe estadístico consolidado', margin, 86);
  pdf.setDrawColor(255, 255, 255, 0.3);
  pdf.setLineWidth(0.3);
  pdf.line(margin, 110, pW - margin, 110);
  pdf.setFontSize(9);
  pdf.setTextColor(180, 200, 220);
  pdf.setFont('helvetica', 'normal');
  pdf.text(`Período: ${periodLabel}     Región: ${regionLabel}`, margin, 118);
  pdf.text(`Generado: ${now}`, margin, 125);

  const totalGreen  = DATA.complianceByRegion.reduce((s, r) => s + r.verde, 0);
  const totalYellow = DATA.complianceByRegion.reduce((s, r) => s + r.amarillo, 0);
  const totalRed    = DATA.complianceByRegion.reduce((s, r) => s + r.rojo, 0);
  const avgComp     = Math.round(DATA.complianceByRegion.reduce((s, r) => s + r.pct, 0) / DATA.complianceByRegion.length);
  const kpis = [
    { label: 'Usuarios', value: String(DATA.summary.totalUsuarios), color: [58, 110, 165] },
    { label: 'En Verde',  value: String(totalGreen),  color: [0, 168, 168] },
    { label: 'Amarillo', value: String(totalYellow), color: [0, 153, 255] },
    { label: 'En Rojo',  value: String(totalRed),    color: [13, 42, 77]  },
    { label: 'Cumpl.',   value: `${avgComp}%`,       color: [108, 92, 231] },
  ];
  const kpiW = (pW - margin * 2 - 12) / 5;
  kpis.forEach((k, i) => {
    const x = margin + i * (kpiW + 3);
    pdf.setFillColor(30, 65, 110);
    pdf.roundedRect(x, 140, kpiW, 22, 2, 2, 'F');
    pdf.setDrawColor(...k.color);
    pdf.setLineWidth(0.8);
    pdf.line(x, 140, x, 162);
    pdf.setTextColor(...k.color);
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text(k.value, x + 5, 152);
    pdf.setFontSize(7);
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(170, 190, 210);
    pdf.text(k.label, x + 5, 158);
  });
...
// ─── DIALOG EXPORTAR PDF ──────────────────────────────────────────────────────
const ExportDialog = ({ open, onClose, onExport, globalFilters, dateRanges }) => {
      ...
      <Paper sx={{ p: 3, width: 440, borderRadius: '12px', maxHeight: '88vh', overflowY: 'auto' }}>
        ...
        <Paper elevation={0} sx={{ p: 1.5, bgcolor: '#f8f9fa', mb: 2, borderRadius: '8px' }}>
          <Typography variant="caption" sx={{ color: C.text.secondary }}>
            Período: <strong>{dateRanges.find(d => d.value === globalFilters.dateRange)?.label}</strong>
            &nbsp;·&nbsp; Región: <strong>{globalFilters.region === 'all' ? 'Todas' : globalFilters.region}</strong>
          </Typography>
        </Paper>
        ...
        <Stack spacing={0.5} sx={{ mb: 3 }}>
          {EXPORT_OPTIONS.map(o => (
            <Box key={o.id} onClick={() => !loading && toggle(o.id)}
              sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1, borderRadius: '6px', cursor: loading ? 'default' : 'pointer', bgcolor: sel.has(o.id) ? '#e8f0fe' : 'transparent', '&:hover': { bgcolor: loading ? undefined : '#f0f4f8' } }}>
              <Box sx={{ width: 18, height: 18, border: `2px solid ${sel.has(o.id) ? C.primary.main : '#ccc'}`, borderRadius: '4px', bgcolor: sel.has(o.id) ? C.primary.main : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {sel.has(o.id) && <Box sx={{ width: 8, height: 8, bgcolor: 'white', borderRadius: '2px' }} />}
              </Box>
              <Typography variant="body2" sx={{ color: C.primary.dark, fontWeight: sel.has(o.id) ? 600 : 400 }}>{o.label}</Typography>
            </Box>
          ))}
        </Stack>
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: { xs: 1, sm: 2, md: 3 } }}>
      ...
      <ExportDialog
        ...
        globalFilters={{ dateRange, region: isAllRegions ? 'all' : selectedRegions.join(', ') }}
